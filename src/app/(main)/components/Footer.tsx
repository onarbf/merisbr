...
export default function Footer({ projects }: Props) {
    const projectNames = projects.map((project) => {
        return <div key={project._id}>{project.name}</div>
    })
    return (<header className="flex flex-col sticky bottom-0 left-0">
        {projects && <div className="flex">
            <h1 className=" flex items-center flex-shrink-0 pl-4 py-2 text-4xl relative z-10 bg-black text-white gap-2">
            ...
            <div className="relative flex overflow-x-hidden bg-black text-white">
                ...
                <div className="flex gap-2">
                    {projects.map((project)=>{
                        return <div key={project._id} className="flex-shrink-0 text-4xl flex">{project.name} <div className="mx-2">|</div></div>
                    })}
                </div>
                </div>
                <div className="py-2 animate-marquee whitespace-nowrap ">
        ...
        <div className="flex items-center justify-center py-2 px-8  bg-white">
            <ul className="flex gap-2">
                <li className="w-[30px] h-[30px] border rounded"></li>
                <li className="w-[30px] h-[30px] border rounded"></li>
                <li className="w-[30px] h-[30px] border rounded"></li>
                <li className="w-[30px] h-[30px] border rounded"></li>
            </ul>
